...
ofstream fnew;
void NewRequest(Ptr<App> app, uint32_t seq)
{
	Ptr<Node> pn = app->GetNode();
	fnew<<Simulator::Now().ToDouble(Time::S)<<"\t"<<Names::FindName(pn)<<"\t"<<app->GetId()<<"\t"<<seq<<endl;
...
//    Config::ConnectWithoutContext(oss.str(), MakeCallback(&LinkOnPacket));
  ...
  consumerHelper.SetAttribute ("ReTxMax", IntegerValue (retxN));
  ...
  for (NodeContainer::Iterator node = consumerNodes.Begin (); node != consumerNodes.End (); node++)
     {
       ...
       pn->GetApplication(0)->TraceConnectWithoutContext("NewRequest", MakeCallback(&NewRequest));
     }
  ...
  string root= "examples/shock/output";
  if (debug == "true" || debug=="True")
	  root += "-debug";
  string s = root + "/cdn-over-ip/Case/latency-"+id+".txt";
    boost::tuple< boost::shared_ptr<std::ostream>, std::list<Ptr<ndn::AppDelayTracer> > >
    tracers = ndn::AppDelayTracer::InstallAll (s);

    s = root + "/cdn-over-ip/Case/jetty-"+id+".txt";
    fnew.open(s.c_str());
    fnew<<"Time\tNode\tAppid\tSeq"<<endl;

    s = root + "/cdn-over-ip/Case/request-"+id+".txt";
